...
public class FourPieceAuto extends SequentialCommandGroup {
  ...
  public FourPieceAuto(  Infeed m_infeed, Shooter m_shooter, CommandSwerveDrivetrain m_drivebase) {
     infeed = m_infeed;
    shooter = m_shooter;
    ...
    addCommands(
      ...
      new StopAll(infeed, shooter),
      new RotatePivotGround(infeed),
      new ParallelCommandGroup(drivebase.getAutoCommand(Constants.Trajectorys.getNote1), new Intake(infeed)).until(infeed::getLimitSwitchReverse),
      new StopAll(infeed, shooter),
      new ParallelCommandGroup(drivebase.getAutoCommand(Constants.Trajectorys.backToSpeaker), new RotatePivotShooter(infeed), new SpeakerShot(shooter)),
      new Expel(infeed),
      new WaitCommand(.2),
      new StopAll(infeed, shooter),
      new RotatePivotGround(infeed),
      new ParallelCommandGroup(drivebase.getAutoCommand(Constants.Trajectorys.getNote2), new Intake(infeed)).until(infeed::getLimitSwitchReverse),
      new StopAll(infeed, shooter),
      new ParallelCommandGroup(drivebase.getAutoCommand(Constants.Trajectorys.backToSpeaker), new RotatePivotShooter(infeed), new SpeakerShot(shooter)),
      new Expel(infeed)
